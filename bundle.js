(() => {
  // js/scripts/edition-registry.js
  var _currentEditionId = "trouble_brewing";
  var _editions = {};
  function registerEdition(id, editionModule) {
    _editions[id] = editionModule;
  }
  function setCurrentEdition(id) {
  ...
  function getRoleHints() {
    return getCurrentEdition().roleHints || {};
  }
  var ROLE_HINTS = new Proxy({}, { get(_, prop) {
    return getRoleHints()[prop];
  } });
  function getRoleStrategyTips() {
    return getCurrentEdition().roleStrategyTips || {};
  }
  var ROLE_STRATEGY_TIPS = new Proxy({}, {
    get(_, prop) {
      if (prop === Symbol.iterator || prop === Symbol.toPrimitive) return void 0;
      const tips = getRoleStrategyTips();
      if (prop === "length") return Object.keys(tips).length;
      return tips[prop];
    }
  });
  function getEvilRoleNames() {
    return getCurrentEdition().roles.filter((r) => r.team === "minion" || r.team === "demon").map((r) => r.name);
  }
  var EVIL_ROLE_NAMES = new Proxy([], {
    get(_, prop) {
  ...
  function getInfoFormatHints() {
    return getCurrentEdition().infoFormatHints || {};
  }
  var INFO_FORMAT_HINTS = new Proxy({}, { get(_, prop) {
    return getInfoFormatHints()[prop];
  } });
  function getFullRoleRules() {
    const ed = getCurrentEdition();
    return ed.roles.map((role) => `${role.name}\uFF08${TEAM_LABEL[role.team] || role.team}\uFF09\uFF1A${role.ability}`).join("\uFF1B");
  }
  function getSlayerDeclarationTemplate() {
    return getCurrentEdition().slayerDeclarationTemplate || "";
  }
  function getSlayerDeclarationNotice() {
  ...
  function getGoodGuidelines() {
    return getCurrentEdition().goodGuidelines || "";
  }
  function getEvilGuidelines() {
    return getCurrentEdition().evilGuidelines || "";
  }
  function getPlayerSystemPrompt() {
    const ed = getCurrentEdition();
  ...
  function getPlayerJsonSystemPrompt() {
    const ed = getCurrentEdition();
    const fullRules = getFullRoleRules();
    const slayerNotice = getSlayerDeclarationNotice();
    return [
      `\u4F60\u662F\u300A\u8840\u67D3\u949F\u697C\xB7${ed.name}\u300B\u7684\u73A9\u5BB6\u3002`,
  ...
  function emptyPlayer(index) {
    return {
      ...
      assassinUsed: false,
      courtierUsed: false,
      courtierDrunkTargetId: "",
      courtierDrunkRemaining: 0,
      professorUsed: false,
      devilsAdvocateTargetId: "",
      exorcistLastTargetId: "",
      grandchildId: "",
      gossipStatement: "",
      gossipStatementDay: 0,
      sailorDrunkTargetId: "",
      poLastAttacked: true,
      pukkaVictimId: "",
      ...
      cursedByWitch: false,
      madAboutRole: "",
      philosopherGainedRole: "",
      philosopherDrunkTargetId: "",
      seamstressUsed: false,
      artistUsed: false,
      evilTwinPairId: "",
      sweetheartDrunkTargetId: "",
      fangGuSwapped: false,
      barberDiedToday: false,
      juggleGuesses: [],
      vigormortisDeadMinions: [],
      noDashiiPoisonedIds: [],
      editionFlags: {}
    };
  }
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target1":"\u73A9\u5BB6\u540D","target2":"\u73A9\u5BB6\u540D"}`;
  ...
  async function storytellerChooseInfo(player, label, trueInfo, fallbackOptions, context = null) {
    ...
    const instructionParts = [
      `\u4F60\u662F\u300A\u8840\u67D3\u949F\u697C\xB7${getCurrentEdition().name}\u300B\u7684\u8BF4\u4E66\u4EBA\u3002`,
      `\u300A\u8840\u67D3\u949F\u697C\xB7${getCurrentEdition().name}\u300B\u662F\u4E00\u6B3E\u8FDB\u9636\u7248\u793E\u4EA4\u63A8\u7406\u6E38\u620F\uFF0C\u53EF\u7406\u89E3\u4E3A"\u6BCF\u4E2A\u4EBA\u90FD\u6709\u72EC\u7279\u8D85\u80FD\u529B\u7684\u72FC\u4EBA\u6740"\u3002`,
  ...
  function maybeAlterNumber(value, maxValue) {
  ...
  var _editionResolvers = {};
  function registerEditionNightResolver(editionId, fn) {
...
\u4F60\u7684\u5F53\u524D\u72B6\u6001\uFF1A${player.alive ? "\u5B58\u6D3B" : "\u6B7B\u4EA1"}\u3002
\u65F6\u95F4\u89C4\u5219\uFF1A${dayRuleNote || "\u65E0"}
${state.phase === "day" && state.dayStage === "discussion" && !state.ended ? `\u730E\u624B\u58F0\u660E\u89C4\u5219\uFF1A\u82E5\u8981\u89E6\u53D1\u5F00\u67AA\uFF0C\u6574\u53E5\u5FC5\u987B\u4E25\u683C\u4E3A"${getSlayerDeclarationTemplate()}"\u3002
` : ""}\u4F60\u7684\u79C1\u5BC6\u4FE1\u606F\u589E\u91CF\uFF1A${privateInfo}
${extraInstruction ? `\u989D\u5916\u7EA6\u675F\uFF1A${extraInstruction}
...
\uFF08\u5DF2\u8BC6\u522B\uFF0C\u53EF\u76F4\u63A5\u786E\u8BA4\u6216\u4FEE\u6539\uFF09`, spoken);
  ...
  var BAD_MOON_RISING = {
    ...
    newbieGuide: [
      `\u4F60\u6B63\u5728\u6E38\u73A9\u300A\u8840\u67D3\u949F\u697C\xB7\u9EEF\u6708\u521D\u5347\u300B\uFF0C\u8FD9\u662F\u4E00\u6B3E\u8FDB\u9636\u7248\u793E\u4EA4\u63A8\u7406\u6E38\u620F\u3002`,
  ...
  var SECTS_AND_VIOLETS = {
    ...
    newbieGuide: [
      `\u4F60\u6B63\u5728\u6E38\u73A9\u300A\u8840\u67D3\u949F\u697C\xB7\u68A6\u6B92\u6625\u5BB5\u300B\uFF0C\u8FD9\u662F\u524D\u4E09\u4E2A\u5267\u672C\u4E2D\u6700\u75AF\u72C2\u7684\u4E00\u4E2A\u3002`,
  ...
  function getTempValue4() {
    const el = document.getElementById("tempInput");
    return el ? Number(el.value) || 1 : 1;
  }
  function normalizeTargetName3(name) {
    if (!name) return "";
    return String(name).replace(/\(真人\)/g, "").replace(/（已死亡）/g, "").trim();
  }
  function resolveTargetByName2(name, candidates, actor) {
    if (!name || !candidates.length) return null;
    const normalized = normalizeTargetName3(name);
    if (normalized === "\u81EA\u5DF1" || normalized === "\u6211" || normalized === "\u6211\u81EA\u5DF1") {
      return actor && candidates.find((p) => p.id === actor.id) ? actor : null;
    }
    return candidates.find((p) => p.name === normalized) || null;
  }
  async function aiChooseSingleTarget2(actor, candidates, actionLabel, extraNote = "") {
    if (!actor || !candidates.length) return null;
    const privateInfo = formatPrivateInfoForPrompt(actor, "json", 4);
    const privateChatHistory = formatPlayerPrivateChats(actor);
    const recentChat = formatChatForPrompt(12, actor, "json");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D"}`;
    ...
    try {
      const content = await callDeepSeek(prompt2, getTempValue4(), actor, "json");
      const json = extractJson(content);
      const target = json ? resolveTargetByName2(json.target, candidates, actor) : null;
      return target || null;
    } catch (error) {
      return null;
    }
  }
  async function aiChooseTwoTargets2(actor, candidates, actionLabel) {
    if (!actor || candidates.length < 2) return candidates;
    const privateInfo = formatPrivateInfoForPrompt(actor, "json", 4);
    const privateChatHistory = formatPlayerPrivateChats(actor);
    const recentChat = formatChatForPrompt(12, actor, "json");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target1":"\u73A9\u5BB6\u540D","target2":"\u73A9\u5BB6\u540D"}`;
    ...
    try {
      const content = await callDeepSeek(prompt2, getTempValue4(), actor, "json");
      const json = extractJson(content);
      const t1 = json ? resolveTargetByName2(json.target1, candidates, actor) : null;
      const t2 = json ? resolveTargetByName2(json.target2, candidates, actor) : null;
  ...
  function getTeaLadyProtection() {
    ...
    if (!teaLady || isDroisoned(teaLady)) return /* @__PURE__ */ new Set();
  ...
  async function resolveNightBMR() {
    if (!state.started || state.phase !== "night") return;
    if (state.paused) return;
    if (needsHumanNightAction() && !isHumanActionReady()) {
      addChat("\u7CFB\u7EDF", "\u8BF7\u5148\u786E\u8BA4\u4F60\u7684\u591C\u665A\u884C\u52A8\u3002", "system");
    ...
    try {
      ...
      let killedThisNight = [];
      let wokenRoles = /* @__PURE__ */ new Set();
      let exorcised = false;
      if (sailor) {
        wokenRoles.add("\u6C34\u624B");
        let target = null;
        if (human && human.roleName === "\u6C34\u624B" && state.humanActionTarget) {
          ...
          const candidates = state.players.filter((p) => p.alive);
          target = await aiChooseSingleTarget2(sailor, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\uFF1A\u4F60\u6216\u4ED6\u9189\u9152");
      ...
      if (innkeeper && state.nightCount > 1) {
        ...
        if (human && human.roleName === "\u65C5\u5E97\u8001\u677F" && state.humanActionTarget && state.humanActionTarget2) {
          ...
          const candidates = state.players.filter((p) => p.alive);
          targets = await aiChooseTwoTargets2(innkeeper, candidates, "\u9009\u62E9\u4E24\u540D\u73A9\u5BB6\u4FDD\u62A4\uFF08\u5176\u4E2D\u4E00\u4EBA\u9189\u9152\uFF09");
        }
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D","guess":"\u89D2\u8272\u540D"}`,
          ...
          try {
            const content = await callDeepSeek(prompt2, getTempValue4(), gambler, "json");
            const json = extractJson(content);
            if (json && json.target && json.guess) {
              const target = state.players.find((p) => p.name === normalizeTargetName3(json.target));
              if (target) {
      ...
      if (devilsAdvocate) {
        ...
        if (human && human.roleName === "\u9B54\u9B3C\u4EE3\u8A00\u4EBA" && state.humanActionTarget) {
          ...
          target = await aiChooseSingleTarget2(devilsAdvocate, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\uFF1A\u660E\u5929\u4ED6\u88AB\u5904\u51B3\u4E0D\u4F1A\u6B7B");
      ...
      if (exorcist && state.nightCount > 1) {
        ...
        if (human && human.roleName === "\u9A71\u9B54\u4EBA" && state.humanActionTarget) {
          ...
          target = await aiChooseSingleTarget2(exorcist, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\uFF08\u4E0D\u80FD\u91CD\u590D\u4E0A\u591C\uFF09\uFF1A\u82E5\u9009\u4E2D\u6076\u9B54\u5219\u6076\u9B54\u4E0D\u884C\u52A8");
      ...
      if (demon && state.nightCount > 1 && !exorcised) {
        if (demonRole === "\u50F5\u6016") {
          if (!state.deathsToday || state.deathsToday === 0) {
            wokenRoles.add("\u50F5\u6016");
            let target = await aiChooseSingleTarget2(demon, state.players.filter((p) => p.alive || p.id === demon.id), "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u6B7B\u4EA1\uFF08\u50F5\u6016\u51FB\u6740\uFF0C\u4EC5\u5F53\u767D\u5929\u65E0\u4EBA\u6B7B\u4EA1\u65F6\uFF09");
          ...
          if (state.pukkaVictimId) {
            ...
            if (oldVictim) {
              oldVictim.poisoned = false;
            }
          }
          let target = await aiChooseSingleTarget2(demon, state.players.filter((p) => p.alive && p.id !== demon.id), "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u4E2D\u6BD2\uFF08\u666E\u5361\uFF09");
          if (!target) target = chooseRandomTarget(demon, false);
          ...
          if (demon.shabalothLastTargets && demon.shabalothLastTargets.length) {
            const deadTargets = demon.shabalothLastTargets.map((id) => state.players.find((p) => p.id === id && !p.alive)).filter(Boolean);
          ...
          const targets = await aiChooseTwoTargets2(demon, candidates, "\u9009\u62E9\u4E24\u540D\u73A9\u5BB6\u6B7B\u4EA1\uFF08\u6C99\u5DF4\u6D1B\u65AF\uFF09");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target1":"\u73A9\u5BB6\u540D","target2":"\u73A9\u5BB6\u540D","target3":"\u73A9\u5BB6\u540D"}`,
            ...
            try {
              const content = await callDeepSeek(prompt2, getTempValue4(), demon, "json");
              const json = extractJson(content);
              if (json) {
                const t1 = resolveTargetByName2(json.target1, candidates, demon);
                const t2 = resolveTargetByName2(json.target2, candidates, demon);
                const t3 = resolveTargetByName2(json.target3, candidates, demon);
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D"} \u6216 {"target":"\u4E0D\u653B\u51FB"}`,
            ...
            try {
              ...
              if (json && json.target && json.target !== "\u4E0D\u653B\u51FB") {
                const target = resolveTargetByName2(json.target, state.players.filter((p) => p.alive), demon);
                if (target) demonTargets = [target];
                demon.poLastAttacked = true;
              } else {
      ...
      if (demon && state.nightCount === 1 && demonRole === "\u666E\u5361") {
        wokenRoles.add("\u666E\u5361");
        let target = await aiChooseSingleTarget2(demon, state.players.filter((p) => p.alive && p.id !== demon.id), "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u4E2D\u6BD2\uFF08\u666E\u5361\u9996\u591C\uFF09");
        if (!target) target = chooseRandomTarget(demon, false);
        if (target && !isDroisoned(demon)) {
          target.poisoned = true;
          target.poisonedUntilDay = 999;
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"use":true,"target":"\u73A9\u5BB6\u540D"} \u6216 {"use":false}`,
          ...
          try {
            const content = await callDeepSeek(prompt2, getTempValue4(), assassin, "json");
            const json = extractJson(content);
            if (json && json.use) {
              target = resolveTargetByName2(json.target, candidates, assassin);
      ...
      if (godfather && state.nightCount > 1 && state.outsiderDiedToday && !isDroisoned(godfather)) {
        ...
        if (human && human.roleName === "\u6559\u7236" && state.humanActionTarget) {
          ...
          const candidates = state.players.filter((p) => p.alive);
          target = await aiChooseSingleTarget2(godfather, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u6B7B\u4EA1\uFF08\u6559\u7236\u51FB\u6740\uFF0C\u56E0\u767D\u5929\u6709\u5916\u6765\u8005\u6B7B\u4EA1\uFF09");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"use":true,"target":"\u73A9\u5BB6\u540D"} \u6216 {"use":false}`,
          ...
          try {
            const content = await callDeepSeek(prompt2, getTempValue4(), professor, "json");
            const json = extractJson(content);
            if (json && json.use) {
              const target = resolveTargetByName2(json.target, deadPlayers, professor);
              if (target) {
...
\u8BF7\u5224\u65AD\u8FD9\u4E2A\u58F0\u660E\u662F\u5426\u6B63\u786E\uFF0C\u5982\u679C\u6B63\u786E\u8BF7\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u6B7B\u4EA1\u3002`,
        ...
        if (json && json.isTrue) {
          const target = state.players.find((p) => p.name === normalizeTargetName3(json.target) && p.alive);
      ...
      if (chambermaid) {
        ...
        if (human && (human.roleName === "\u4F8D\u5973" || human.drunk && human.apparentRoleName === "\u4F8D\u5973") && state.humanActionTarget && state.humanActionTarget2) {
          ...
          targets = await aiChooseTwoTargets2(chambermaid, candidates, "\u9009\u62E9\u4E24\u540D\u73A9\u5BB6\uFF0C\u5F97\u77E5\u5176\u4E2D\u51E0\u4EBA\u4ECA\u665A\u56E0\u81EA\u8EAB\u80FD\u529B\u88AB\u5524\u9192");
        }
        if (targets.length === 2) {
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"narration":"\u591C\u665A\u53D9\u8FF0","publicAnnouncement":"\u767D\u5929\u516C\u5F00\u4FE1\u606F"}` }
      ...
      let publicAnnouncement = "";
      try {
        const content = await callDeepSeek(storytellerPrompt, getTempValue4());
        const json = extractJson(content);
        if (json) {
          narration = json.narration || "";
          publicAnnouncement = json.publicAnnouncement || "";
  ...
  // js/scripts/sv-night.js
  function getTempValue5() {
    const el = document.getElementById("tempInput");
    return el ? Number(el.value) || 1 : 1;
  }
  function normalizeTargetName4(name) {
    if (!name) return "";
    return String(name).replace(/\(真人\)/g, "").replace(/（已死亡）/g, "").trim();
  }
  function resolveTargetByName3(name, candidates, actor) {
    if (!name || !candidates.length) return null;
    const normalized = normalizeTargetName4(name);
    if (normalized === "\u81EA\u5DF1" || normalized === "\u6211" || normalized === "\u6211\u81EA\u5DF1") {
      return actor && candidates.find((p) => p.id === actor.id) ? actor : null;
    }
    return candidates.find((p) => p.name === normalized) || null;
  }
  async function aiChooseSingleTarget3(actor, candidates, actionLabel, extraNote = "") {
    if (!actor || !candidates.length) return null;
    const privateInfo = formatPrivateInfoForPrompt(actor, "json", 4);
    const privateChatHistory = formatPlayerPrivateChats(actor);
    const recentChat = formatChatForPrompt(12, actor, "json");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D"}`;
    ...
    try {
      const content = await callDeepSeek(prompt2, getTempValue5(), actor, "json");
      const json = extractJson(content);
      return json ? resolveTargetByName3(json.target, candidates, actor) : null;
    } catch (error) {
      return null;
    }
  }
  function getNoDashiiPoisonTargets(demon) {
    ...
    for (let d = 1; d < total; d++) {
      ...
      if (p.team === "townsfolk" && p.id !== demon.id) {
        result.push(p);
        break;
      }
    }
  ...
  async function resolveNightSV() {
    ...
    try {
      ...
      if (demonRole === "\u8BFA-\u8FBE\u9CBA" && demon && !isDroisoned(demon)) {
        state.players.forEach((p) => {
          if (p.noDashiiPoisonedIds) p.poisoned = false;
        });
      ...
      if (snakeCharmer) {
        ...
        if (human && human.roleName === "\u821E\u86C7\u4EBA" && state.humanActionTarget) {
          ...
          target = await aiChooseSingleTarget3(snakeCharmer, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\uFF1A\u82E5\u9009\u4E2D\u6076\u9B54\u5219\u4EA4\u6362\u89D2\u8272\u548C\u9635\u8425");
        }
      ...
      if (witch) {
        ...
        if (aliveCount > 3) {
          ...
          if (human && human.roleName === "\u5973\u5DEB" && state.humanActionTarget) {
            ...
            const candidates = state.players.filter((p) => p.alive);
            target = await aiChooseSingleTarget3(witch, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u8BC5\u5492\uFF1A\u82E5\u4ED6\u660E\u5929\u63D0\u540D\u5219\u6B7B\u4EA1");
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D","role":"\u89D2\u8272\u540D"}`,
        ...
        try {
          const content = await callDeepSeek(prompt2, getTempValue5(), cerenovus, "json");
          const json = extractJson(content);
          if (json && json.target && json.role && !isDroisoned(cerenovus)) {
            const target = resolveTargetByName3(json.target, candidates, cerenovus);
            if (target) {
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"target":"\u73A9\u5BB6\u540D","role":"\u89D2\u8272\u540D"}`,
          ...
          try {
            const content = await callDeepSeek(prompt2, getTempValue5(), pitHag, "json");
            const json = extractJson(content);
            if (json && json.target && json.role && !isDroisoned(pitHag)) {
              const target = resolveTargetByName3(json.target, candidates, pitHag);
      ...
      if (currentDemon && state.nightCount > 1) {
        ...
        if (human && human.team === "demon" && state.humanActionTarget) {
          ...
          target = await aiChooseSingleTarget3(currentDemon, candidates, `\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\u6B7B\u4EA1\uFF08${currentDemonRole}\u51FB\u6740\uFF09`);
        ...
        if (target && target.alive) {
          if (currentDemonRole === "\u65B9\u53E4" && target.team === "outsider" && !currentDemon.fangGuSwapped) {
            ...
            if (target.roleName === "\u5FC3\u4E0A\u4EBA" && !isDroisoned(target)) {
              ...
              if (json && json.target) {
                const drunkTarget = state.players.find((p) => p.name === normalizeTargetName4(json.target) && p.alive);
                if (drunkTarget) {
                  drunkTarget.drunk = true;
      ...
      if (dreamer) {
        ...
        if (human && (human.roleName === "\u7B51\u68A6\u5E08" || human.drunk && human.apparentRoleName === "\u7B51\u68A6\u5E08") && state.humanActionTarget) {
          ...
          target = await aiChooseSingleTarget3(dreamer, candidates, "\u9009\u62E9\u4E00\u540D\u73A9\u5BB6\uFF1A\u5F97\u77E5\u4E00\u4E2A\u5584\u826F\u89D2\u8272\u548C\u4E00\u4E2A\u90AA\u6076\u89D2\u8272\uFF08\u5176\u4E2D\u4E00\u4E2A\u662F\u4ED6\u7684\u771F\u5B9E\u89D2\u8272\uFF09");
        }
        if (target) {
          const ed = getCurrentEdition();
...
\u8BF7\u8F93\u51FA JSON\uFF1A{"narration":"\u591C\u665A\u53D9\u8FF0","publicAnnouncement":"\u767D\u5929\u516C\u5F00\u4FE1\u606F"}` }
      ...
      let publicAnnouncement = "";
      try {
        const content = await callDeepSeek(storytellerPrompt, getTempValue5());
        const json = extractJson(content);
        if (json) {
          narration = json.narration || "";
          publicAnnouncement = json.publicAnnouncement || "";
  ...
  if (editionSelect) {
    ...
    if (state && state.editionId) {
      try {
        setCurrentEdition(state.editionId);
