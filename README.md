...
> 🎮 **想直接玩？** 如果你选择 [在线游玩](#方法一在线游玩最简单)，只需要一个浏览器和一个 `API Key`，**无需安装任何环境**，可以跳过下面的环境准备和安装步骤，直接看 [🚀 启动游戏](#-启动游戏)。

以下是本地部署或运行 Benchmark 评测所需的环境：
...
> 💡 如果你选择 [在线游玩](#方法一在线游玩最简单)，可以跳过整个安装步骤。
...
无需下载代码、无需安装环境、无需配置文件，直接访问我们部署的在线版本：
