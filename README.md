...
- 🎭 **完整的暗流涌动剧本**：包含全部 13 个镇民、4 个外来者、4 个爪牙和 1 个恶魔角色，完整还原桌游体验
- 🤖 **多模型 AI 玩家**：支持 DeepSeek、Gemini、Claude、GPT、MiMo、OpenRouter 等多家大模型提供商，每个 AI 玩家可以使用不同的模型
- 🧠 **AI 说书人（LLM Storyteller）**：说书人决策由大模型驱动，根据游戏局势智能生成信息和裁决，替代硬编码逻辑
- 😈 **邪恶阵营密聊（Evil Chat）**：第一个白天全体公聊前，恶魔、爪牙之间可以进行内部密聊协调策略，以使邪恶阵营能够更充分地交换信息
- 📋 **每日 AI 总结**：每个白天结束时，AI 自动为每位玩家生成当日总结，防止长局 context 超限，同时保留关键推理信息。总结并行生成，不阻塞游戏进程
- 🗣️ **AI 语音播报 (TTS)**：基于 mimo-v2-tts 引擎，每位 AI 玩家拥有独特音色风格
- 🎵 **BGM 歌单系统**：内置 15 首背景音乐，按氛围分类（开场、讨论、推理、入夜、决赛、结局），自动营造沉浸感
- 🎙️ **语音输入**：支持浏览器语音识别，用说话代替打字
- 💬 **公聊 + 私聊**：第一个白天可以私聊交换线索，后续白天公开讨论
...
- **你的名字**：设置你的角色名称
- **座位**：设置你的座位号
- **你的角色**：从下拉菜单选择你想扮演的角色，或选择「随机」由系统随机分配（注意：不能手动选择酒鬼角色）
- **「生成玩家」**：根据人数生成所有玩家
- **「随机发牌」**：为所有人分配角色
- **「开局」**：开始游戏
- **「随机模型」**：为每个 AI 玩家随机分配不同的 AI 模型

**2. 模型配置**
- **默认模型**：选择默认使用的 AI 模型
- **温度**：调节 AI 的"创造力"（0-1，数值越高越随机）
- **白天讨论时长**：设置每个白天讨论阶段的时长（1-30 分钟，默认 8 分钟）
- **模型健康检查**：显示各模型配置是否正确、Key 是否有效

**3. 游戏控制**
- **自动夜晚结算**：开启后夜晚会自动结算，无需手动点击
- **记录 LLM 轨迹**：开启后记录所有 AI 调用的详细信息（含 system prompt）
- **暂停 / 夜晚结算 / AI轮转 / 强制提名 / 投票结算 / 结束游戏**：各种手动控制按钮

**4. TTS 语音配置**
- **AI 语音播报开关**：开启后 AI 发言会自动语音播放
- **语音音量**：调节语音播报音量

### 主游戏区域

**左侧 - 城镇广场**
- **座位圆环**：以环形展示所有玩家的座位、名字、状态（存活/死亡）。点击某个玩家的座位可以为该玩家标注你猜测的角色,点击某个玩家旁边的➕号可以为该玩家标注你认为的状态和标签，帮助你整理推理线索。
- **配置玩家模型**：为每个 AI 玩家单独选择使用的模型
- **配置摘要**：显示当前游戏的角色分配情况
- **Token 用量**：显示各模型的调用次数和费用

**右侧 - 聊天与操作**
- **公开聊天**：所有玩家的公开发言，支持按玩家筛选、关键词搜索
- **私聊**：仅第一个白天可用，可以和指定玩家私聊交换线索
- **发言输入框**：输入你想说的话
- **@提及**：可以 @ 某个玩家引起他的注意
- **猎手格式按钮**：当你是猎手时出现，帮你生成标准格式的猎手宣言
- **语音输入**：点击后用麦克风说话，自动转为文字
- **记录板**：显示游戏的重要事件记录（谁死了、谁被提名等）
...
1. **打开游戏页面**后，你会看到一个欢迎界面，点击 **「开始游戏」** 按钮
2. 会播放一段开场视频（可以点击 **「跳过视频」** 跳过）
3. 点击左上角的 **⚙️ 齿轮按钮**，打开设置面板
4. 设置基本信息：
   - **人数**：建议新手先选 **7-8 人**（人少节奏快，容易上手）
   - **你的名字**：随便取一个名字
   - **座位**：选择你想坐的座位号
   - **你的角色**：如果想体验特定角色可以选择，留空则随机分配
5. 选择 **默认模型**（从下拉菜单中选一个你配置了 API Key 的模型）
6. 按顺序点击：
   - **「生成玩家」** → 生成所有玩家位置
   - **「随机发牌」** → 为所有人分配角色
   - **「开局」** → 正式开始游戏！

> 💡 **小技巧**：点击 **「随机模型」** 可以让每个 AI 玩家使用不同的模型，让对话风格更丰富！

### 二、第一个夜晚（首夜）

1. 开局后自动进入第一个夜晚
2. 说书人会按照角色顺序依次"唤醒"各角色执行能力：
   - **投毒者**选择一个人下毒
   - **间谍**查看魔典信息
   - **洗衣妇/图书管理员/调查员/厨师**获得首夜信息
   - **共情者**获得邻居邪恶数量
   - **占卜师**选择两个人查验
   - ......
3. **如果你的角色有夜晚行动**（如占卜师、僧侣等），右侧「你的夜晚行动」区域会出现操作界面，按提示选择目标即可
4. **首夜恶魔不杀人**，所以第一个白天不会有人死亡
5. 如果开启了「自动夜晚结算」，夜晚会自动完成；否则需要手动点击 **「夜晚结算」**

### 三、邪恶阵营密聊（Evil Chat）

> 🆕 这是新增的游戏环节，首夜结束后、第一个白天全体公聊前，恶魔、爪牙之间可以进行内部密聊协调策略，以使邪恶阵营能够更充分地交换信息。

1. 首夜结束后、第一个白天公开讨论之前，邪恶阵营（恶魔 + 爪牙）会进行一轮 **内部密聊**
2. 密聊期间每个邪恶玩家轮流发言，最多发 3 条消息
3. 密聊期间善良阵营无法发言，界面会显示 **「邪恶阵营密聊中...」** 的提示和倒计时
4. 密聊结束后自动进入正常白天讨论
5. **如果你是邪恶阵营玩家**，你可以在密聊阶段与队友交流策略；如果双方都是邪恶阵营，允许互相暴露真实身份

> 💡 这个机制让邪恶阵营有机会在第一天讨论前统一口径，更贴近真实桌游体验。

### 四、白天讨论

1. 天亮后，说书人会播报昨晚的死亡信息（首夜没有死亡）
2. 进入 **讨论阶段**：
   - AI 玩家会按照座位顺序轮流发言
   - **轮到你发言时**，在底部输入框输入你想说的话，点击 **「发送发言」**
   - 你也可以点击 **「跳过讨论」** 放弃本轮发言
   - 你可以使用 **@ 功能** 点名某个玩家，他会对你的发言做出回应
3. **第一个白天的特殊功能 — 私聊**：
   - 切换到 **「私聊」** 标签
   - 选择你想私聊的玩家，输入内容，点击 **「发送私聊」**
   - 私聊内容只有你和对方看得到，可以用来交换线索、建立信任
   - ⚠️ 只有第一个白天可以私聊，后续白天只能公开讨论！
4. 讨论有时间限制（默认 8 分钟），倒计时结束后自动进入提名阶段
5. **每日总结**：每个白天结束时，系统会自动为每位玩家并行生成 AI 总结，压缩历史对话以保留关键推理信息，防止长局中 context 超限

### 五、提名与投票（黄昏）

1. 讨论结束后进入 **提名阶段**：
   - 每个存活玩家（包括你）都有机会提名一个人
   - **玩家可以提名自己**（例如主动跳身份求验证）
   - **轮到你提名时**：从下拉菜单选择你想提名的玩家，点击 **「提名」**
   - 不想提名可以点击 **「跳过提名」**
   - 每人仅一次提名机会，每人最多被提名一次
2. 有人被提名后，进入 **投票阶段**：
   - 提名者陈述理由，被提名者进行辩解发言
   - 然后所有人按顺序投票
   - **轮到你投票时**：点击 **✓ 赞成** 或 **✗ 反对**（也可以使用键盘快捷键：**Y** = 赞成，**N** = 反对）
   - 得票数 ≥ 存活人数一半的被提名者会 **上处决台**
   - **处决台上**得票最多的那个人被处决（票数相同则无人被处决）
3. 投票结束后，如果有人被处决，说书人会宣布处决结果
4. 然后进入下一个夜晚

### 六、后续夜晚

1. 从第二夜开始，**小恶魔会选择一名玩家杀害**
2. 有夜晚行动的角色继续执行能力（僧侣保护、占卜师查验等）
3. 天亮后说书人会宣布 **昨晚谁死了**（但不会公布死者的角色身份）

### 七、特殊情况

- **猎手开枪**：如果你是猎手，白天讨论时可以使用 **「猎手格式」** 按钮生成开枪宣言，格式为 "我是猎手，我要向玩家X开枪"。如果目标是恶魔，恶魔立即死亡！每局只能用一次。
- **死亡玩家**：死亡后仍然可以发言讨论，而且还有 **一次死人票**（整局只能投一次）
- **贞洁者被提名**：如果提名贞洁者的人是镇民，提名者会立刻被处决
- **圣徒被处决**：如果圣徒被投票处决，善良阵营立刻失败
- **镇长日**：如果只剩 3 名玩家存活且白天无人被处决，善良阵营获胜
- **邪恶阵营自我暴露**：邪恶阵营玩家之间允许互相暴露真实身份

### 八、游戏结束

游戏会在以下情况结束：

- ✅ **善良胜利**：恶魔被处决（或猎手击杀恶魔），善良阵营获胜
- ✅ **善良胜利**：触发镇长日（仅剩 3 人且无人处决）
- ❌ **邪恶胜利**：场上只剩 2 名存活玩家且恶魔仍然存活
- ❌ **邪恶胜利**：圣徒被处决

游戏结束后：
- 会显示 **复盘面板**，展示所有玩家的真实角色、夜晚行为、聊天记录等
- 可以点击 **「生成复盘」** 让 AI 说书人写一段游戏回顾
- 可以点击 **「导出复盘」** 保存完整游戏记录
- 进入 **赛后聊天** 模式：游戏结束后你仍然可以继续发言，与 AI 玩家讨论刚才的对局。点击 **「AI轮转」** 按钮可以让 AI 玩家们也发表赛后感言
