...
> 一款基于 AI 的血染钟楼单人网页游戏，你将作为唯一的真人玩家，与 AI 说书人和 AI 玩家一起体验经典社交推理剧本「暗流涌动」。
...
- 🤖 **多模型 AI 玩家**：支持 DeepSeek、Gemini、Claude、GPT、OpenRouter 等多家大模型提供商，每个 AI 玩家可以使用不同的模型
...
- 📋 **完整的游戏流程**：夜晚结算 → 天亮播报 → 白天讨论 → 提名投票 → 处决，全自动或半自动进行
...
1. 讨论结束后进入 **提名阶段**：
   ...
   - 不想提名可以点击 **「跳过提名」**
   - 每人仅一次提名机会，每人最多被提名一次
2. 有人被提名后提名者陈述理由，被提名者反驳辩解，接着进入 **投票阶段**：
   - 提名者陈述理由，被提名者进行辩解发言
   - 然后所有人按顺序投票
...
本项目基于桌游《血染钟楼》(Blood on the Clocktower) 的规则，使用大语言模型 (LLM) 驱动 AI 说书人和 AI 玩家，实现了单人可以体验完整游戏流程的 Demo。

脚本为「暗流涌动」(Trouble Brewing)，这是《血染钟楼》最经典的入门级剧本，适合新手学习游戏规则。

**技术栈**：纯前端实现（HTML + CSS + JavaScript），无需后端服务器，通过浏览器直接调用各大模型的 API。
